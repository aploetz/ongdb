...
object SemanticTableConverter {
  ...
  def convertSemanticTable(table: SemanticTableV3_4, expressionMapping: ExpressionMapping4To5): SemanticTable = {
    new SemanticTable(
  ...
  ASTAnnotationMap[expressionsv3_6.Expression, ExpressionTypeInfo] = {
  ...
  private def convert(typeInfoV3_4: semanticsV3_4.ExpressionTypeInfo): ExpressionTypeInfo = typeInfoV3_4 match {
    case semanticsV3_4.ExpressionTypeInfo(specified, expected) =>
      semantics.ExpressionTypeInfo(convert(specified), expected.map(convert))
  }
