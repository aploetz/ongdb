...
import org.neo4j.cypher.internal.v3_6.{ast, expressions}
...
trait Command extends Parser
  ...
  def PropertyExpressions: Rule1[Seq[org.neo4j.cypher.internal.v3_6.expressions.Property]] = rule("multiple property expressions") {
    oneOrMore(WS ~ expressions.Variable ~ PropertyLookup, separator = CommaSep)
  }
  ...
  private def NodeKeyConstraintSyntax: Rule3[Variable, LabelName, Seq[org.neo4j.cypher.internal.v3_6.expressions.Property]] = keyword("CONSTRAINT ON") ~~ "(" ~~ expressions.Variable ~~ NodeLabel ~~ ")" ~~
    keyword("ASSERT") ~~ "(" ~~ PropertyExpressions ~~ ")" ~~ keyword("IS NODE KEY")

  private def UniqueConstraintSyntax: Rule3[Variable, LabelName, org.neo4j.cypher.internal.v3_6.expressions.Property] = keyword("CONSTRAINT ON") ~~ "(" ~~ expressions.Variable ~~ NodeLabel ~~ ")" ~~
    keyword("ASSERT") ~~ PropertyExpression ~~ keyword("IS UNIQUE")

  private def UniqueCompositeConstraintSyntax: Rule3[Variable, LabelName, Seq[org.neo4j.cypher.internal.v3_6.expressions.Property]] = keyword("CONSTRAINT ON") ~~ "(" ~~ expressions.Variable ~~ NodeLabel ~~ ")" ~~
    keyword("ASSERT") ~~ "(" ~~ PropertyExpressions ~~ ")" ~~ keyword("IS UNIQUE")

  private def NodePropertyExistenceConstraintSyntax = keyword("CONSTRAINT ON") ~~ "(" ~~ expressions.Variable ~~ NodeLabel ~~ ")" ~~
